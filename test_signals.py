...
def demo():
    ...
    assert m.bias("AAA", 100.0) == ("+0.0%", "dim"), "貼著均線是中性"
    assert m.bias("AAA", 110.0)[1] == "hot", "乖離 +10% 要標追高"
    assert m.bias("AAA", 90.0)[1] == "cold", "乖離 -10% 要標回檔"
    assert m.bias("AAA", 105.0)[1] == "dim", "+5% 還在中間帶"
    assert m.bias("NOPE", 10.0) == ("", ""), "沒日線資料就留白"
    assert m.pos52("AAA", 100.0)[0] == "50%", "高低點正中間是 50%"
    assert m.pos52("AAA", 150.0) == ("100%", "up"), "創高是 100%"
    assert m.pos52("AAA", 50.0) == ("0%", "down"), "貼底是 0%"
    ...
    assert m.chip_streak("2330.TW") == ("+3日", "up"), "連三天買超"
    assert m.chip_streak("2317.TW") == ("-1日", "down"), "只有最近一天賣超"
    ...
    # 警示：每天每條只響一次，過期自動消失
    m.quotes.clear(); m.alert_msgs.clear(); m.alert_fired.clear(); mk.PAGES = {}
    mk.bell = False
    m.quotes["AAA"] = (100.0, 95.0)
    mk.ALERTS = [{"symbol": "AAA", "name": "測試", "above": 99}]
    m.check_alerts()
    assert len(m.alert_msgs) == 1 and mk.bell, "突破要報一次並響鈴"
    mk.bell = False
    m.check_alerts()
    assert len(m.alert_msgs) == 1 and not mk.bell, "同一天同一條不重複報"
